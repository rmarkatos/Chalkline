...
(async () => {
  ...
  r = await tryQuery(db,
    `insert into public.checks (class_id, student_id, marks) values ('algebra2', 'user_amy', '{"Problem 1": true}'::jsonb)
     on conflict (class_id, student_id) do update set marks = excluded.marks returning marks`);
  chk('teacher can mark one workspace', r.ok && r.rows.length === 1 && r.rows[0].marks['Problem 1'] === true, r.err);
  ...
  r = await tryQuery(db,
    `update public.checks set marks = '{"Problem 1": true}'::jsonb where student_id='user_amy' returning *`);
  chk('a student CANNOT mark their own work', !r.ok || r.rows.length === 0, r.ok ? 'changed ' + r.rows.length : '');
  r = await tryQuery(db, `select public.my_teacher_name() as n`);
